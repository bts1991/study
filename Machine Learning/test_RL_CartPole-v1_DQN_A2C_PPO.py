...
# 하이퍼파라미터
EPISODES = 3 # 하나의 에피소드는 막대기를 넘어뜨리거나 최대 시간(500 step)에 도달할 때까지의 시퀀스
GAMMA = 0.99
...
class DQNAgent:
    def __init__(self):
        ...
        self.epsilon = 0.5 #1.0 # 행동 선택 시 무작위 선택할 확률. 처음엔 100% 탐색부터 시작
    ...
    def act(self, state):
        ...
        print('torch.state: ', torch.FloatTensor(state))
        print('torch.state.shape: ', torch.FloatTensor(state).shape)
        print('torch.state+unsqueeze: ', state)
        print('torch.state+unsqueeze.shape: ', state.shape)
        print('action2-1: ', self.q_net(state))
        print('action2-2: ', self.q_net(state).argmax())
        print('action2-3: ', self.q_net(state).argmax().item())
